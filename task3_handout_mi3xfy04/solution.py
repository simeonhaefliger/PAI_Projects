...
class BO_algo(object):
    def __init__(self):
        ...
        self.constraint_kernel = ConstantKernel(constant_value=3.5, constant_value_bounds="fixed") * RBF(length_scale=2, length_scale_bounds="fixed")
        self.objective_kernel = ConstantKernel(constant_value=1.5, constant_value_bounds="fixed") * RBF(length_scale=1.5, length_scale_bounds="fixed")
    ...
    def acquisition_function(self, x: np.ndarray) -> np.ndarray:
        ...
        if len(self.previous_points) == 0:
            f_x_best = self.objective_model.sample_y(x.reshape(1, -1))
        else:
            ind = np.argmin(np.array([i[2] for i in self.previous_points]))
            f_x_best = self.previous_points[ind][2]
        
        pred_mean_c, pred_var_c = self.constraint_model.predict(x.reshape(1, -1), return_std=True)
        constraint_likelihood = norm.cdf(0, loc = pred_mean_c, scale = pred_var_c)
        
        pred_mean_obj, pred_var_obj = self.objective_model.predict(x.reshape(1, -1), return_std=True)
        prob_of_improvement = (f_x_best - pred_mean_obj)/np.sqrt(pred_var_obj)
        expected_improvement = np.sqrt(pred_var_obj) * (prob_of_improvement * norm.cdf(prob_of_improvement) + norm.pdf(prob_of_improvement))
        
        af_value = expected_improvement * constraint_likelihood
        
        return float(np.ravel(af_value))
    ...
    def add_data_point(self, x: np.ndarray, z: float, c: float):
        ...
        # TODO: enter your code here
        x0 = [i[0] for i in self.previous_points]
        x1 = [i[1] for i in self.previous_points]
        X = np.column_stack((x0, x1))
        yo = np.array([i[2] for i in self.previous_points])
        yc = np.array([i[3] for i in self.previous_points])
        self.objective_model.fit(X, yo)
        self.constraint_model.fit(X, yc)
    ...
    def get_solution(self) -> np.ndarray:
        ...
        # TODO: enter your code here
        # ind = np.argmin(np.array([i[2] for i in self.previous_points]))
        # x_opt = self.previous_points[ind][0:2]
        f = [i[2] for i in self.previous_points]
        c = [i[3] for i in self.previous_points]
        inds = [i for i in range(len(c)) if c[i] <= 0]
        
        if len(inds) == 0:
            ind = np.argmin(f)
        else:
            ind = inds[np.argmin([f[i] for i in inds])]
